...
int main()
{
	...
	for (int simulationCount = 1; simulationCount <= 10000; simulationCount++)
	{
		...
		//Uncomment section for each diffrent distrabtuion
		...
		*/
		...
		float X2 = (float)distribution(generator) / 10000000;
		X2 = (X2 > .2) ? .3 : 1.3; // Whooooo, A conditional operator. I don't think I'll ever use one of these again, too hard to read
		*/

		//case4 v2
		/*
		float X1 = (float)distribution(generator) / 10000000;
		if (X1 > .2 ) X1 = .3; else X1 = 1.3;
		
		float X2 = (float)distribution(generator) / 10000000;
		if (X2 > .2 ) X2 = .3; else X2 = 1.3;
		*/
